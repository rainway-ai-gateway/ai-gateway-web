...
> **前置条件**：须先在 [03 章 模型服务商](03-model-provider.md) 中创建好服务商（实例池、协议、模型、Keys），再在集群向导中选择「所属服务商」。
...
| 字段 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- |
| 负载均衡模式 | 否 | `WRR（加权轮询）` | `WRR`：BFE 本地加权轮询；`EPP`：由 EPP 调度器接管后端选择（实例分配见 [05 章 EPP 调度](05-epp-schedule.md)） |
...
- 删除集群前须解除路由规则引用。
- EPP 模式集群的后端选择由 EPP 调度器接管；若实例池中无有效分配（未分配态），该集群导出配置时会**降级为 WRR**。分配关系在「EPP调度」页查看与覆写，见 [05 章](05-epp-schedule.md)。
