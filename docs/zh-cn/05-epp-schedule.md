...
EPP（Endpoint Picker）是独立于 BFE 的**后端调度器**。集群将「负载均衡模式」设为 `EPP` 后，后端实例的选择不再由 BFE 本地加权轮询完成，而是交给 EPP 调度器接管（模式与调度参数在集群向导步骤 4 的「均衡模式配置」卡片中设置，见 [4.6 节](04-ai-business-cluster.md#均衡模式配置)）。
